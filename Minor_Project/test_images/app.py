...
def detect_disease_yolo(image: Image.Image):
    ...
    if label_lower == "healthy":
        ...
        severity, disease = "Severe", "Downy Mildew"
    else:
        severity, disease = "Healthy", "Healthy"
    return {
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_upload:
        ...
        # ── Clear all state when file changes or is removed ────────────────────
        ...
        if current_file != prev_file:
            ...
            st.session_state["detection_done"]    = False   # ← new flag

        if uploaded:
            image = Image.open(uploaded).convert("RGB")

            # ── Display overlay OR original ────────────────────────────────────
            ...
            if st.button("🔍 Run Detection Pipeline", use_container_width=True):
                ...
                # ── Stage 3: Auto-generate overlay for diseased leaves ─────────
                ...
                if disease_found:
                    with st.spinner("🔬 Stage 3: Mapping affected regions..."):
                        try:
                            ...
                            # Overwrite with pixel-accurate measurement
                            ...
                            # Non-fatal — detection result still saved, overlay just won't show
            ...
            # ── Completion message (persists after rerun via session state flag) ─
...
#         <div class="hero-sub">Reference images showing healthy and Downy Mildew-infected spinach leaves at various severity stages</div>
#     </div>
#     """, unsafe_allow_html=True)
#     st.markdown("""
#     <div class="card" style="background:#f7fbf8; border-left:4px solid #2d7a4f;">
...
#         Compare your field samples against these reference images to identify Downy Mildew at different stages.
#         Early detection (Mild stage) is critical for effective management. If you see symptoms matching
#         Moderate or Severe stages, apply fungicide treatment immediately and notify your agronomist.
#         </p>
#     </div>
#     """, unsafe_allow_html=True)
#     st.markdown("<div class='section-header'>🟢 Healthy Spinach</div>", unsafe_allow_html=True)
#     st.markdown("""
#     <div class="card">
#         <div class="card-title">What Healthy Spinach Looks Like</div>
#         <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-bottom:1rem;">
#             <div style="background:#e6f7ed; border:1px solid #a8e6bf; border-radius:10px; padding:1rem; text-align:center;">
#                 <div style="font-size:2.5rem; margin-bottom:0.5rem;">🍃</div>
#                 <div style="font-weight:700; color:#1a7a40; font-size:0.9rem;">Upper Surface</div>
#                 <div style="font-size:0.8rem; color:#444; margin-top:4px;">Deep, uniform green; smooth texture; no spots or lesions</div>
#             </div>
#             <div style="background:#e6f7ed; border:1px solid #a8e6bf; border-radius:10px; padding:1rem; text-align:center;">
#                 <div style="font-size:2.5rem; margin-bottom:0.5rem;">🌿</div>
#                 <div style="font-weight:700; color:#1a7a40; font-size:0.9rem;">Lower Surface</div>
#                 <div style="font-size:0.8rem; color:#444; margin-top:4px;">Pale green, visible veins, NO grey or purple growth</div>
#             </div>
#             <div style="background:#e6f7ed; border:1px solid #a8e6bf; border-radius:10px; padding:1rem; text-align:center;">
#                 <div style="font-size:2.5rem; margin-bottom:0.5rem;">📐</div>
#                 <div style="font-weight:700; color:#1a7a40; font-size:0.9rem;">Leaf Shape</div>
#                 <div style="font-size:0.8rem; color:#444; margin-top:4px;">Flat, normal size, no curling or puckering</div>
#             </div>
#         </div>
#     </div>
#     """, unsafe_allow_html=True)
...
#     st.markdown("<div class='section-header'>🔴 Downy Mildew — Severity Stages</div>", unsafe_allow_html=True)
#     stages = [
#         ("🟡 Stage 1 — Mild (Early)", "#e8b84b", "#fffbf0", "#f9e79f",
#          "DSI: 0–1.5 · Affected: <10%",
#          ["Pale water-soaked spots appear on upper leaf surface","Spots are angular, bounded by leaf veins",
#           "No visible sporulation yet on lower surface","Plant appears mostly normal from distance"],
#          "Scout carefully; begin preventive fungicide application now", "~8% yield loss if untreated"),

#         ("🟠 Stage 2 — Moderate", "#e07b39", "#fff5ef", "#f9cba3",
#          "DSI: 1.5–3.5 · Affected: 10–40%",
#          ["Yellow-green angular lesions clearly visible","Early grey sporulation visible on leaf undersides",
#           "Some leaf curling and distortion beginning","Multiple lesions per leaf; some leaf yellowing"],
#          "Apply Metalaxyl-M + Mancozeb immediately; repeat in 7–10 days", "~25% yield loss if untreated"),

#         ("🔴 Stage 3 — Severe", "#d94f3d", "#fef0ef", "#f5b7b1",
#          "DSI: 3.5–5.0 · Affected: >40%",
#          ["Dense grey-purple sporulation covering lower leaf surface","Brown necrotic lesions on upper surface",
#           "Heavy leaf curling, wilting, and premature senescence","Significant reduction in leaf size and fresh weight"],
#          "Emergency fungicide application; consider crop destruction in hotspot areas", "~55% yield loss if untreated"),
#     ]


#     for title, color, bg, border_color, dsi_info, symptoms, action, loss in stages:
#         st.markdown(f"""
#         <div class="card" style="border-left:5px solid {color}; background:{bg};">
#             <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:10px;">
#                 <div>
#                     <div style="font-family:Playfair Display,serif; font-size:1.1rem; font-weight:700; color:{color};">{title}</div>
#                     <div style="font-size:0.78rem; color:#666; margin-top:2px;">{dsi_info}</div>
#                 </div>
#                 <div style="background:{border_color}; border:1px solid {color}; border-radius:8px; padding:6px 14px; font-size:0.78rem; color:{color}; font-weight:700;">{loss}</div>
#             </div>
#             <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-top:12px;">
#                 <div>
#                     <div style="font-size:0.75rem; color:#888; font-weight:600; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px;">Symptoms</div>
#                     {''.join([f'<div style="font-size:0.82rem; color:#444; padding:3px 0; border-bottom:1px solid {border_color};">🔸 {s}</div>' for s in symptoms])}
#                 </div>
#                 <div style="background:white; border-radius:8px; padding:10px; border:1px solid {border_color};">
#                     <div style="font-size:0.75rem; color:#888; font-weight:600; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px;">Field Image</div>
#                     <div style="background:#f0f5f1; border-radius:6px; height:100px; display:flex; align-items:center; justify-content:center; font-size:0.78rem; color:#aaa; text-align:center; padding:10px;">
#                         📁 Place field photos in<br/><code>samples/diseased/</code>
#                     </div>
#                     <div style="margin-top:8px; background:#f7fbf8; border-left:3px solid {color}; padding:6px 10px; font-size:0.78rem; color:#444;">
#                         💊 {action}
#                     </div>
#                 </div>
#             </div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_a:
        ...
        <div class="card">
            ...
            <p style="color:#444; line-height:1.9; font-size:0.92rem;">This project is part of ongoing plant pathology research at the Department of Plant Pathology. The study focuses on:</p>
    ...
    with col_b:
        ...
        <div class="card" style="background:linear-gradient(135deg,#0d2818,#1a4a2e); color:white;">
            <div style="font-family:Playfair Display,serif; font-size:1.1rem; color:#a8e6bf; margin-bottom:1rem;">Research Guide</div>
            <div style="text-align:center; padding:1rem 0;">
                <div style="font-size:3rem; margin-bottom:0.5rem;">👩‍🔬</div>
                <div style="font-family:Playfair Display,serif; font-size:1.2rem; font-weight:700; color:#fff;">Dr. Madhuri Pant</div>
                <div style="font-size:0.82rem; color:#6ab880; margin-top:4px;">Plant Pathology Lab</div>
                <div style="font-size:0.78rem; color:#88b89a; margin-top:2px;">Western Maharashtra Research Centre</div>
            </div>
            <hr style="border-color:#2a5c3a; margin:1rem 0;"/>
            <div style="font-size:0.82rem; color:#d4eadb; line-height:1.9;">
                <b style="color:#a8e6bf;">Specialization:</b><br/>
                Spinach disease epidemiology<br/>
                Oomycete pathogen biology<br/>
        ...
        <div class="card">
            ...
            <div style="text-align:center; padding:0.5rem 0;">
                ...
                <div style="font-family:Playfair Display,serif; font-size:1rem; font-weight:700; color:#1a4a2e;">Downy Mildew</div>
                <div style="font-size:0.82rem; color:#888; font-style:italic;">Peronospora effusa</div>
            </div>
            <div style="font-size:0.8rem; color:#444; line-height:1.8; margin-top:0.5rem;">
                🔸 Obligate biotrophic pathogen<br/>
                🔸 Polycyclic disease (rapid spread)<br/>
                🔸 5–10 day incubation period<br/>
                🔸 Up to 55% yield loss (severe)<br/>
                🔸 Peak season: Dec–Jan (Maharashtra)
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="card">
        <div style="font-family:DM Mono,monospace; font-size:0.82rem; color:#1a4a2e; background:#f0f5f1; border-radius:10px; padding:1.5rem; line-height:2.0;">
        📤  User uploads spinach leaf image<br/>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↓<br/>
        🌿  <b>Stage 1 — YOLO Classifier (class_2/best.pt)</b><br/>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;→ NOT spinach? → ❌ Reject<br/>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;→ DSI, Yield loss, AUDPC, Heatmap overlay<br/>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↓<br/>
    ...
    <div class="card" style="background:#fff8f0; border-left:4px solid #e8b84b;">
        ...
        PalakGuard is a <b>research tool</b> intended to support, not replace, professional plant pathological diagnosis. Predictions are probabilistic and should be verified with field observations.
        Always consult your local agricultural extension officer or KVK before making major crop management decisions.
        AI Facilitator responses are generated by a language model and may require expert validation for critical decisions.
        </p>
    </div>
    """, unsafe_allow_html=True)
